...
const (
	name           = "product_type"
	name_ru        = "product type"
	name_ru_plural = "product types"
	...
	menu_icon       = "image/product_type.png"
...
func GetDoc(project *t.ProjectType) t.DocType {
	doc := t.DocType{
		...
		Flds: []t.FldType{
			t.GetFldTitle(),
			t.GetFldString("slug", "slug", 100, [][]int{{1, 2}}),
			t.GetFldString("old_id", "old_id", 100, [][]int{{2, 1}}).SetVif("false").SetIsUniq(),
		},
		Vue: t.DocVue{
			RouteName:      name,
			MenuIcon:       menu_icon,
