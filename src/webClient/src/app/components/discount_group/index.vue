...
<template>
  <q-page :padding="!isOpenInDialog">
    <comp-breadcrumb class="text-capitalize" v-if="!isOpenInDialog" :list="[{label: $t('discount_group.name_plural'), docType:'discount_group'}]"/>

    <comp-doc-list ref="docList" :listTitle="$t('discount_group.name_plural')" :listDeletedTitle="$t('discount_group.name_plural_deleted')" pg-method="discount_group_list"
      ...
      <template #listItem="{item}">
		...
		<router-link :to="currentUrl + item.id" style="cursor: pointer">
			<q-item-section avatar>
			  <q-avatar rounded>
				<img src="image/discount_group.png" alt="">
			  </q-avatar>
			</q-item-section>
		</router-link>
        ...
        <q-item-section top side>
          <comp-item-dropdown-btn :item="item" itemProp="title" :is-edit="true" :is-delete="!(false || false)" fkProp=""
                                  pg-method="discount_group_update"
