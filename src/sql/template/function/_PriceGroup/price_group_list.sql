...
DROP FUNCTION IF EXISTS price_group_list(params JSONB);
CREATE OR REPLACE FUNCTION price_group_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from price_group as doc ' || condQueryStr || ')
