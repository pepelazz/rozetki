...
DROP FUNCTION IF EXISTS serie_list(params JSONB);
CREATE OR REPLACE FUNCTION serie_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from serie as doc ' || condQueryStr || ')
