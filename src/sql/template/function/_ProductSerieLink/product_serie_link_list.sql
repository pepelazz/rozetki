-- получение списка product_serie_link
-- параметры:
...
DROP FUNCTION IF EXISTS product_serie_link_list(params JSONB);
CREATE OR REPLACE FUNCTION product_serie_link_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'product_id', 'doc.product_id'],
		['notQuoted', 'serie_id', 'doc.serie_id']
    ]);
    ...
    EXECUTE ('
	with t1 as (select * from product_serie_link as doc ' || condQueryStr || ')
