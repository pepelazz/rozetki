...
DROP FUNCTION IF EXISTS customer_list(params JSONB);
CREATE OR REPLACE FUNCTION customer_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    whereStr = where_str_build(params, 'doc', ARRAY [
		...
		['notQuoted', 'brand_id', 'doc.brand_id']
    ]);
    ...
    EXECUTE ('
	with t1 as (select * from customer as doc ' || condQueryStr || ')
