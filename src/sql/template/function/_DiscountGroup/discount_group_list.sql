-- получение списка discount group
-- параметры:
...
DROP FUNCTION IF EXISTS discount_group_list(params JSONB);
CREATE OR REPLACE FUNCTION discount_group_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from discount_group as doc ' || condQueryStr || ')
