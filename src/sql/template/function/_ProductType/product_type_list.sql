...
DROP FUNCTION IF EXISTS product_type_list(params JSONB);
CREATE OR REPLACE FUNCTION product_type_list(params JSONB)
    RETURNS JSON
    LANGUAGE plpgsql
AS
$function$
...
BEGIN
    ...
    EXECUTE ('
	with t1 as (select * from product_type as doc ' || condQueryStr || ')
